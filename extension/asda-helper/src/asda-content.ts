(() => {
  ...
  function findIncreaseButton(container: HTMLElement) {
    return Array.from(container.querySelectorAll<HTMLButtonElement | HTMLElement>("button, [role='button']")).find((button) => {
      ...
      const text = `${button.textContent ?? ""} ${button.getAttribute("aria-label") ?? ""} ${button.getAttribute("title") ?? ""} ${button.getAttribute("data-testid") ?? ""} ${button.getAttribute("data-auto-id") ?? ""}`
        ...
        .trim()
        .toLowerCase();
      if (/(increase|increment|add one|plus|more|quantity up)/.test(text)) return true;
      return text === "+" || text === "＋";
    });
  }
