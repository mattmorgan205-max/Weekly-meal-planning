(() => {
  ...
  function findConfidentAddButton() {
    const candidates = Array.from(document.querySelectorAll<HTMLButtonElement | HTMLElement>("button, [role='button']"))
      .filter((element) => {
    ...
    if (isProductPage && candidates.length > 0) return candidates[0];
